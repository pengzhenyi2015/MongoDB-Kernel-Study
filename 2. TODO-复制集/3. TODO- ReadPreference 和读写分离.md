...
## 2.1 mongos内核代码分析
__节点状态信息采集__     
每个 mongos 节点都维护一个 ReplicaSetMonitor 探测并维护每个 shard 的拓扑信息，默认每隔 [30 秒](https://github.com/mongodb/mongo/blob/r4.0.3/src/mongo/client/replica_set_monitor.cpp#L169)进行一次周期性探测。另外，为了避免探测周期太长导致维护的拓扑信息存在过期数据的问题，在 mongos 根据 readPreference 选择节点时，如果没有合适的节点也会[主动触发探测](https://github.com/mongodb/mongo/blob/r4.0.3/src/mongo/client/replica_set_monitor.cpp#L273) 。探测的实现方式是向 mongod 节点发送 isMaster 命令，获取节点的状态、配置以及网络距离信息：    
```
try {
