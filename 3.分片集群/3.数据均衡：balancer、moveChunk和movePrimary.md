...
5. A -> B：前面第2步提到了， A 节点一直在给 B 节点发送 _recvChunkStatus 询问状态。到目前为止，B 节点终于变为 "steady" 了，那么 A 节点也知道自己应该进入 catchup 状态了。进入 catchup 阶段的典型特征就是：设置这个表只读 (5.0内核版本还引入了 [maxCatchUpPercentageBeforeBlockingWrites参数](https://www.mongodb.com/zh-cn/docs/manual/release-notes/5.0/#maxcatchuppercentagebeforeblockingwrites-server-parameter)进一步控制停写的时机, 用于平衡停写时间和整体的迁移时间)。下面描述具体细节：    
   ...
   - b. 然后 A 节点的迁移流程，标记自己进入了 criticalSection 状态。并调用 [updateShardCollectionsEntry](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/s/shard_metadata_util.cpp#L205) 通知自己的 secondary 节点（具体执行方式是将 cache.collections 中对应的 enterCriticalSectionCounter 字段自增，并[通过 oplog 通知](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/s/shard_server_op_observer.cpp#L287) secondary）， secondary 后续收到读请求时会进行阻塞式路由更新。      
...
### 安全性保证
在 chunkRange 被迁移走之后，并不能立即删除。比如：    
1. 会被较老的快照读引用。    
2. 这个 chunkRange 又在被迁移回来。比如 chunk1 从 shard A 迁移到了B，但是没有马上删除。然后在很短的时间内，又从 B 迁移回 A。而此时 A 上的孤儿文档删除任务刚好调度到了 chunk1 的删除。那么此时能够一边从 B->A 迁移 chunk1，一边又在 A 上清理 chunk1 吗？    
...
movePrimary 并不保证线程安全性，即迁移期间最好不要有读写请求，否则不做任何服务保证。参考[官方文档](https://www.mongodb.com/zh-cn/docs/manual/reference/command/moveprimary/)：
