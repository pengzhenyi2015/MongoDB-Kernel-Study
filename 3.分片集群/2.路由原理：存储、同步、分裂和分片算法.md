...
4. 根据分割点数组进行微调，比如：
    ...
    - b. 避免创建过小的 chunk.
    - c. 如果要分裂的 chunk 位于表的第一个 chunk 或者最后一个 chunk，则将分割点向极值靠拢。这个特性主要是针对 range 分片时，按照 shardKey 的顺序进行写入的场景。
...
2. 读取本地的 config.chunks 表，获取当前分片表的 collectionVersion 和 shardVersion. 由于 config.chunks 表有基于 ns 和 lastmod 的索引，因此读取成本很低。    
