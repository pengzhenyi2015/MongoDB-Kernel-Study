...
在 5.0 内核版本之后，引入了时序表特性。时序表的插入操作都会转换为对应 bucket 的 update 操作（bucket 的第一条数据除外）。为了在列式压缩下尽量提升复制性能，对 o 字段进行了扩展，通过存储 oplog diff 的方式，减少 oplog 大小（不需要用 $set 记录修改后的全量值），并提升 apply 性能（直接应用，不需要解压和重新压缩）。举例如下：
```
{
        "op" : "u",
...
WiredTiger 引擎在内存中使用一个[全局链表](https://github.com/mongodb/mongo/blob/r4.2.25/src/third_party/wiredtiger/src/include/txn.h#L156)来维护事务的 durable_timestamp 信息。      
在事务开始时，会将获取到的 oplog ts 作为 timestamp 加入到全局链表中。函数调用的路径为：[LocalOplogInfo::getNextOpTimes](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/repl/local_oplog_info.cpp#L102) (获取 oplog 的 ts) -> oplogDiskLocRegister -> [WiredTigerRecoveryUnit::setTimestamp](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/storage/wiredtiger/wiredtiger_recovery_unit.cpp#L718) （将 ts 作为 commit_timetamp 进行设置） -> __session_timestamp_transaction -> __wt_txn_set_timestamp -> [__wt_txn_publish_timestamp](https://github.com/mongodb/mongo/blob/r4.2.25/src/third_party/wiredtiger/src/txn/txn_timestamp.c#L995) (加入内存队列)。        
当另外一个读事务使用 "all_durable" 查询 oplog 时，会遍历全局链表，找到最小的（没有空洞的）ts，然后以 ts-1 作为 "all_durable" 时间戳（参考 [__txn_global_query_timestamp](https://github.com/mongodb/mongo/blob/r4.2.25/src/third_party/wiredtiger/src/txn/txn_timestamp.c#L240)）。    

当一个事务结束时（提交或者终止），释放资源时会设置事务对应的 `clear_durable_q` 标识。判断 "all_durable" 的流程会跳过这些已经完成的事务，并且其他事务插入链表的流程中对这些链表项进行清理。从而达到 "all_durable" 时间戳一直推进的效果。    
