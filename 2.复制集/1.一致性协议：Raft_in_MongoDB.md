...
|方案|应对的场景|基本思路|
|:--|:--|:--|
|Log Read|网络分区|先为读操作也生成 1 条no-op log，走一遍  Raft 的提交流程，宣示 leader 的主权 。然后再执行读操作。<br>这样可以防止出现网络分区后，仍然在旧 leader上读数据的情况。<br>这种方案比较直观。但是读操作的流程较长，性能会比较差。另外通过日志提交将读操作线性化后，读并发会受到影响。<br>**MongoDB 的线性一致性设计采用了这个思想。**|
|Read Index|网络分区|Leader 收到读请求后先记录 readIndex = committeIndex，然后**通过心跳宣示主权**，确认自己不是少数派。然后确保 appliedIndex >= readIndex 后再执行读操作。<br>Raft 论文原文：1) Leader commit a blank no-op entry into the log at the start of its term.2) Leader exchange heartbeat messages with a majority of the cluster before responding to read-only requests. |
