...
但是对于通配符索引，由于字段名字不固定，因此索引的 btree 结构中需要存储字段名。具体来说，在创建通配符索引时，会遍历通配符字段下的所有字段（参考 [WildcardKeyGenerator::_traverseWildcardhttps://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/index/wildcard_key_generator.cpp#L108]()）,然后将 indexPath 和 indexKey 一起组成一个索引项（参考 [WildcardKeyGenerator::_addKey](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/index/wildcard_key_generator.cpp#L172)）。       
假设创建了 `{"a.$**":1}`索引， 则会对 a 下属的字段创建如下索引：     

<p align="center">
<img width="1860" height="1172" alt="image" src="https://github.com/user-attachments/assets/22415695-c322-47b5-ac21-992f577c2245" />
</p>
