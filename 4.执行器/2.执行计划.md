...
```
在 MongoDB 内核中，会对数组中的每个字段创建索引，所以虽然上面的例子中只有 3 条文档，但是会对应 1+3+4=8 条索引项。      
在使用 `{a:1}` 进行查询时，会有 4 条对应的索引项满足条件，但是在[**去重**](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/exec/index_scan.cpp#L196)之后会精准返回 3 条文档。      
...
但是对于通配符索引，由于字段名字不固定，因此索引的 btree 结构中需要存储字段名。具体来说，在创建通配符索引时，会遍历通配符字段下的所有字段（参考 WildcardKeyGenerator::_traverseWildcard）,然后将 indexPath 和 indexKey 一起组成一个索引项（参考 WildcardKeyGenerator::_addKey）。       
