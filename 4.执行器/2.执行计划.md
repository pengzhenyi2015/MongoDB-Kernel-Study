...
```
// 将经度和维度转换为 unsigned 整型的过程 GeoHashConverter::convertToHashScale -> GeoHashConverter::convertToDoubleHashScale
...
double GeoHashConverter::convertToDoubleHashScale(double in) const {
    ...
    verify(in >= 0);
    return in * _params.scaling;  // 乘以 11930464cm，从弧度映射成 360 * scaling == uint32 最大值（4,294,967,296）， 更详细的可以参考 GeoHashConverter::parseParameters
}
```
示意图如下：
...
// 具体的执行流程，可以理解为，将 uint32 的经纬度信息Hash到 uin64 的 _hash 中，而是每个维度只取高位的 26 bit 信息（也就是说经度从 1cm 左右下降到 60 cm）
...
1. 如何确定起始步长？    
   ...
   上述算法可以参考 [GeoHash::appendVertexNeighbors](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/geo/hash.cpp#L565)。    
