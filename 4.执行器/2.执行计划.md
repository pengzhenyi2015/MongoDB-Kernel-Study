...
>在 insert/update/delete 原生命令中也可以会设置 ordered=false, 导致请求在不同 shard 之间可能乱序，但是在同一个 shard 内执行的请求还是有序的。在同一个 shard 内执行的请求有序，也就意味着同一个 _id 文档的修改是有序的。      
>这也是 BulkWrite 特别需要注意的地方。
