...
### 全局事务 ID
分布式事务一般会涉及多个 shard 节点的本地事务，那么首当其冲的问题就是如何将多个 shard 节点的本地事务串联起来。      
对于很多分布式数据库来说，使用 Global TransactionID (有些系统也叫 GXID) 来完成串联工作。Global TranscationID 一般是一个递增的 uint64 或者某种 UUID。    
>需要说明的是，Global TransactionID 是事务开始阶段创建的，**不具备事务定序的功能**。在事务提交时，事务的 commit_timestamp 才会被确定，才能用于事务的定序。     

MongoDB 内核代码中没有使用传统的 Global TransactionID 概念，但是采用的思路相同：    
1. 如何在多个节点之间串联这个事务？    
...
>**细说 lsid 如何串联事务的多个操作**    
>1. 在同一个 session 下，每次执行事务内的操作会带上 lsid 信息，在命令执行的入口会进行检查和 CheckOut 的动作（参考 [runCommandImpl](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/service_entry_point_common.cpp#L581)->invokeWithSessionCheckedOut）.     
>2. 在 [invokeWithSessionCheckedOut](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/service_entry_point_common.cpp#L365) 中，会根据 lsid 从全局的 [SessionCatalog](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/session_catalog.h#L54) 的 hash 表中得到 session（如果是第一次则创建），然后获取 session 的 TransactionParticipant 装饰器。     
>3. TransactionParticipant 会进行必要的检查（比如 txnNumber 不能比比之前的小）以及统计信息更新（参考 [TransactionParticipant::Participant::beginOrContinue](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/transaction_participant.cpp#L483)）。然后将上一次缓存的事务状态恢复到本次操作的 OperationContext 上下文中，包括事务的锁、以及 WT 事务指针 recoveryUnit 等关键信息（参考 [TransactionParticipant::Participant::unstashTransactionResources](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/transaction_participant.cpp#L958)）.      
>4. 然后执行事务操作，比如对 WT 引擎执行 CRUD 命令。    
>5. 命令执行完毕后，如果事务还没有结束，会再将事务信息缓存起来。等下一个命令执行时，再从缓存中恢复（参考 [TransactionParticipant::Participant::stashTransactionResources](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/transaction_participant.cpp#L886)）。    
...
>**prepare_ts**: 由每个 shard 自行决定，并会写入到存储引擎中。当另外一个事务 B 使用时间戳 ts_b 读数据时，如果发现 ts_b > prepare_ts，那么这个事务 B 会产生 PREPARE_CONFLICT 异常并（等待唤醒之后）重试。因为对于事务 B 来说，这条数据可能可见（后续事务 A 提交并且 ts_b >= commit_ts）,也可能不可见（ts_b < commit_ts 或者事务 A 回滚了）。

>**commit_ts**: 由 coordinator 确定： commit_ts = max(prepare_ts)，遵循 commit_ts >= prepare_ts 的语义。commit_ts 也会写入到存储引擎中。当另外一个事务 B 使用时间戳来读数据时，比如 readConcern=snapshot，使用时间戳 ts_b 读数据。由于对于事务 A 来说，之前在各个 shard 上提交的数据的 commit_ts 都严格相同，因此能够保证事务 B 读取数据的一致性：要么全部可见（ts_b>=commit_ts），要么全部不可见（ts_b < commit_ts）.

>但是对于非事务场景，而且没有严格按照时间戳读取（比如 readConcern 设置为 available、 local、 snapshot 的场景），则可能出现 “半读”的情况： shard1 上读到已经提交的版本，但是 shard2 上读到的是提交之前的旧版本。     
因为，分布式事务虽然最终的 commit_ts 标签在各个 shard 上相同，但是每个 shard 上实际执行 commit 的物理时间有先右后。如果读事务不能保证在每个 shard 上使用相同的 ts 读取数据，则可能出现不一致的情况。
...
但是这个事务此时只是 prepared 状态，并没有提交，如果写 oplog 的动作也放在用户操作的顶层事务中，那么这个 oplog 必然对 secondary 节点不可见。而且由于 oplog 空洞的原因，必然也无法同步后续其他事务的 oplog.    

如何解决这个问题呢？    
显然 prepare 操作对应的 oplog 不能放在用户事务中，需要单独开一个事务去执行。       
...
>注1： SideTransactionBlock 与关系型数据库中常见的**子事务**并不相同。SideTransactionBlock 是一个完全独立的事务，具有独立的生命周期。而子事务是在一个父事务中开启的事务。
...
### MVCC
MVCC 部分请参考前述 [4.1 章节](https://github.com/pengzhenyi2015/MongoDB-Kernel-Study/blob/main/4.%E6%89%A7%E8%A1%8C%E5%99%A8/1.%E9%94%81%E4%B8%8E%E5%B9%B6%E5%8F%91%E6%8E%A7%E5%88%B6.md)中的描述。
