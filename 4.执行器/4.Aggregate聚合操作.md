...
- $bucket 和 $bucketAuto：将文档分桶到指定的字段中。       
- $facet：对文档进行分面聚合，返回多个聚合结果。    
...
其中 data 部分会尝试用 snappy 进行压缩，如果压缩率低于 90% 则存储的是压缩数据，并将 size 置为负数以作标记。     
...
`$lookup` 操作能实现类似关系型数据库中的关联子查询、非关联子查询、left outer join 等功能。       
以下面的操作为例：
```
db.coll1.aggregate({
  $lookup: {
...
```
[执行流程](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_lookup.cpp#L236)为：    
1. 从左表 coll1 中获取下一条文档。
2. 将原始数据提取出 `localField` 并结合 `foreignField` 生成一个 matchStage.
3. 将这个 matchStage，结合 lookup 中指定的其他 pipeline，生成一个新的 pipeline.
4. 对 coll2 表执行这个新的 pipeline，将结果用数组的形式存储到 `as` 字段中。
5. 重复步骤 1-4，直到处理完 coll1 中的所有文档。

### lookup 优化
如果下一个 stage 是 `$unwind`, 而且 unwind 处理的刚好是 `as` 字段，则会将 `$unwind` 合并到 `$lookup` 节点。      
这样可以避免不必要的中间结果转换过程： 读取 coll2 中满足条件的文档 -> 组织成数组的形式传递给后面的 `$unwind` -> `$unwind` 再将数组拆开。

如果既包含 `$unwind`，后续又有针对 `as` 字段的 `$match` 阶段，并且不依赖 `$unwind` 的 includeArrayPath(拆分后的数组下标)和 preserveNullAndEmptyArrays（空数组置为 null） 特性。则可以将 `$match` 也一并吸收。

这种提前过滤和减少中间结果转换的优化，在处理大数量级的数据集时，能够显著提高查询性能。     

### 对比关系型数据库的 join
常见的 join 算法有 3 种：
1. Nested Loop: 通过 2 层循环来遍历 2 个表，时间复杂度 O(N\*M)。如果其中一个表比较小（例如 1000 条记录），则可以将其作为驱动表，将大表作为被驱动表,在大表**有索引**的情况下，时间复杂度为 O(N\*logM)
2. Hash Join: 先对其中一个表构建 hash 表，然后遍历另一个表，根据 hash 表来判断是否匹配，时间复杂度 O(N)。
3. Merge Join: 先对 2 个表进行排序，然后使用 2 个指针来遍历 2 个表，时间复杂度 O(N\*logN)。如果表存在索引，则不需要排序动作，时间复杂度为 O(N)。

从前面的分析可以看出，`$lookup` 属于 nested loop，因此非常依赖被驱动表的索引。**如果表很大，又没有索引，会有灾难级的性能表现**。    

作为对比，关系型数据库的 join 算法都进行了非常深度的优化（可能和 join 操作在关系型数据库要高频很多有关系）。比如在 PostgreSQL 中对上述 3 种 join 算法都进行了实现，并有并行策略的优化，并且在不同的场景下会自动选择最优的算法。可以参考 [hashJoin](https://github.com/postgres/postgres/blob/REL_17_STABLE/src/backend/executor/nodeHashjoin.c#L13) 和 [mergeJoin](https://github.com/postgres/postgres/blob/REL_17_STABLE/src/backend/executor/nodeMergejoin.c#L23) 算法的代码注释。

### 分布式 lookup/join    

MongoDB 高版本支持了分片表的 lookup 操作，但是涉及到跨 shard 关联时，需要生成远端执行计划，**执行逐条查询操作， 此时可能会陷入性能灾难**。    
对于传统关系型数据库来说，会有广播表、数据重分布等方式来加速跨 shard 关联的性能。     
**因此，涉及到大数据量的跨 shard 关联时，需要谨慎选择数据库。**    

## $group
以一个典型的分组聚合操作为例，按照 `a` 字段分组，然后累加 `b` 字段：`{$group: {_id: "$a", total_b: {$sum: "$b"}}}`，其 `$group` 阶段的执行流程为：
1. 第 1 次调用 getNext 时，会执行 [initialize](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L470) 操作，会阻塞式地执行聚合操作，具体为：    
   a. 从输入流获取下一条文档。    
   b. 根据 group 规则计算 group id.    
   c. 根据 group id 找到 Hash 表中的 group 节点。如果不存在，则[创建](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L494)一个新的 group 节点。    
   d. 将文档聚合到 group 节点中。    
   e. 检查 Hash 表的内存使用大小，如果超过了[阈值(默认100MB)](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L476)，则[溢出](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L586)到磁盘文件中。    
   f. 重复步骤 a-e，直到处理完所有文档。    
2. initalize 结束之后，再返回数据，分 2 种情况：    
   a. 如果分组数量比较少，一般在内存 Hash 表就能搞定所有数据，此时会通过 [GroupsMapIterator](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L191) 返回 Hash 表中的数据。    
   b. 如果分组数量比较多，中间结果溢出到磁盘文件中，则通过 MergeIterator 归并所有 FileIterator 的结果再返回。和前面提到的 `$sort` 阶段不同，`$group` 阶段的 merge 操作，需要找到不同 FileIterator 中相同 group id 的中间结果，然后归并时[再聚合一次](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/pipeline/document_source_group.cpp#L164)，合并成**唯一的最终结果**。

示意图如下所示：

<p align="center">
<img width="1234" height="904" alt="image" src="https://github.com/user-attachments/assets/0d46817a-93d3-43f2-a66d-dc3700306c87" />
</p>

## $bucket 和 $bucketAuto    
bucket 用于在指定好分桶边界后，将数据进行分桶聚合。在内核中，会将 `$bucket` 改写成 `$group` + `$sort` 去执行。
比如对于 bucket 请求：
```
{
    groupBy: "$field",
...
$group 阶段:
{
    _id: {
        $switch: {
            branches: [
                { case: {$and: [{$gte: ["$field", 0]}, {$lt: ["$field", 10]}]}, then: 0 },
                { case: {$and: [{$gte: ["$field", 10]}, {$lt: ["$field", 20]}]}, then: 10 },
                { case: {$and: [{$gte: ["$field", 20]}, {$lt: ["$field", 30]}]}, then: 20 }
            ],
            default: "other"
        }
    },
...
bucketAuto 用于自动确定边界，并分成若干个桶进行聚合。和 bucket 改写语句的方式不同，bucketAuto 在内核中有实现完整的处理逻辑，主要包含 2 大步骤：    
1. populateSorter 阶段：阻塞式获取所有的输入文档，记录总文档数，并进行排序。运行方式和 `$sort` 阶段相同。
2. populateBuckets 阶段：根据文档总数和指定的分桶个数，预估每个桶的文档数。然后遍历已经排序的文档，将文档聚合到对应的桶中。


## $facet
facet 用于对数据进行多个分组聚合操作，每个分组聚合操作可以独立进行，互不干扰。facet 的执行流程图如下：     

<p align="center">
<img width="1631" height="427" alt="image" src="https://github.com/user-attachments/assets/ee34a581-f24e-44e7-8090-be70951b71e0" />
</p>

其中 TeeBuffer 用于批量加载底层的输入文档。而上层维护了 facet 所指定的多个 pipeline 操作，并依次对 TeeBuffer 中缓存的数据进行聚合。    
消费完成后，TeeBuffer 会再加载下一批数据，并继续执行聚合操作，直到所有数据消费完毕。    
不过需要注意的是，虽然 TeeBuffer 的缓存很大地减少了对底层数据的读取，但是上层的多个 pipeline 仍然是在同一个线程内执行的，没有并行处理。

## stage 执行优化

和其他数据库一样，MongoDB 内部对执行计划也进行了比较深度的优化，常见的策略和场景有：
1. 提前过滤：将 `$match`、`$sample`、`$limit`、`$skip` 等操作符，尽可能早地放到 pipeline 中执行，减少后续操作的数据量量。举例如下：    
   a. **match 下推**：对于 `[{$addFields: {d: {$add: ["$a", "$b"]}}}, {$match: {a:{$gte:16,$lte:18}, d: {$gte: 32, $lte:36}}}]` 查询，由于 `$addFields` 虽然添加了 `d` 字段，但是没有修改 `a` 字段，因此可以将 `a` 的匹配操作**拆分出来**放在前面执行，这样显著减少 `$addFields` 处理的数据量。最终的执行计划变为：查表时使用 `a:{$gte:16,$lte:18}` 进行过滤，然后执行 `$addFields` 添加 d 字段，然后再使用 `d: {$gte: 32, $lte:36}` 对 `d` 进行过滤。    
   b. **sample 下推**： 对于 `[{$addFields: {d: {$add: ["$a", "$b"]}}}, {$sample: {size: 5}}]` 查询，可以先执行 `$sample`，然后再执行 `$addFields` 添加新字段。    
   c. **limit 下推**： 其中一种典型的场景就是前面提到的 TopK 排序。对于 `[{$addFields: {d: {$add: ["$a", "$b"]}}}, {$limit: 5}]` 查询，可以将 `$limit` 操作符放在前面执行，这样显著减少 `$addFields` 处理的数据量。    
   d. **skip 下推**： 比如 `[{$addFields: {d: {$add: ["$a", "$b"]}}}, {$skip: 5} ]` 查询，可以将 `$skip` 放在前面执行。     
2. 排序消除：尽量利用索引序，减少耗时的 `$sort` 阶段。
3. 减少中间结果：比如将 `$lookup` 与 `$unwind` 合并，避免中间结果转换, 将 `$sort` 和 `$limit` 合并成 TopK 排序等。

MongoDB 内核已经提供了比较强大的优化能力，但是在编写 aggregate 命令时，仍然需要注意的是:     
1. 尽量在命令的编写阶段就进行优化，而不是强依赖内核优化器。这样既方便阅读，也防止出现一些意想不到的问题。比如前面提到的 limit 下推优化，实际测试在 8.2 版本能正常优化，但是在 4.2 版本中则并没有。      
2. 上线之前使用 `explain("executionStats")` 查看执行计划，检查是否符合预期。
...
3. MongoDB 内核代码：https://github.com/mongodb/mongo/tree/r4.2.25/src
4. PostgreSQL 内核代码：https://github.com/postgres/postgres/tree/REL_17_STABLE
