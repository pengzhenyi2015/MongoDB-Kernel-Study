...
### 4.8.1 错误注入
有些测试 case 需要控制多线程之间的时序，或者需要强制触发某种条件。     
此时，可以利用 MongoDB 内核提供的 `MONGO_FAIL_POINT` 错误注入机制。     

使用过程分为 3 步(比如定义一个 printLockTimeout 错误注入标签，用于强制打印锁超时信息)：    
1. 在内核代码中定义一个错误注入标签（建议在cpp文件中，使用位置的附近定义）：
    ```
    namespace {
        MONGO_FAIL_POINT_DEFINE(printLockTimeout);
    }
    ```
    这样，程序启动时，会往全局的 FailPointRegistry 中注册一个错误注入标签。FailPointRegistry 会使用 `stdx::unordered_map<std::string, FailPoint*>` 来存储所有的错误注入标签。

2. 在代码中，使用第 1 步定义好的错误注入标签：
    ```
    if (MONGO_FAIL_POINT(printLockTimeout)) {
        // 打印锁超时信息
    }
    ```

3. 配置 MongoDB 配置文件，启动错误注入命令。然后可以连接 mongodb， 使能或者禁用该错误注入标签：
    ```
    // 修改 MongoDB 配置文件，并重启：
    setParameter:
        enableTestCommands: true

    // 在测试 js 文件中，或者 mongo shell 中，可以开启或者关闭该错误注入标签：
    db.adminCommand({configureFailPoint: "printLockTimeout", mode: "alwaysOn"})
    ```

    关于 configureFailPoint 命令可以指定的参数，可以参考 [FailPoint::parseBSON](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/util/fail_point.cpp#L193) 中的流程。

### 4.8.2 打印开发日志
线上发布的 release 版本，在编译时，需要设置编译选项 `--dbg=off` （默认也是 off）。    
但是，在开发过程中，可以通过设置编译选项 `--dbg=on` 来开启开发日志。另外，开启这个编译选项之后，也会禁用编译器的优化功能，方便调试。       

在开启了 `--dbg=on` 之后，可以通过下面的宏执行一些开发相关的动作（比如增加一些断言，或者打印日志等）：
```
DEV  do_somgthing();
```
...
9. https://github.com/mongodb/mongo/blob/r4.2.25/src
