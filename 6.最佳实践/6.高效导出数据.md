# 1. 导语
在真实的业务场景中，常会遇到如何将一个大表的数据快速导出的问题。对于这种场景，普通的全表扫描由于存在并发受限，往往无法满足业务需求。     
...
## 5.3 使用 expr 比较的聚合操作，实现续传和分块导出    
使用 expr 比较的聚合方式，可以导出类型不同的数据。     
举例如下，假设表中有如下多类型 `_id` 的数据：
```
mymongo:PRIMARY> db.docs.find().sort({_id:1})
{ "_id" : 1, "data" : 1 }
{ "_id" : 5, "data" : 5 }
{ "_id" : 10, "data" : 10 }
{ "_id" : "a", "data" : "a" }
{ "_id" : ISODate("2026-09-10T08:41:58.420Z"), "data" : "xxx" }
```
如果使用 find 或者普通的 match 聚合查询，不会输出类型不同的数据：
```
mymongo:PRIMARY> db.docs.aggregate([{$match: {_id: {$gte:10}}}])
{ "_id" : 10, "data" : 10 }
mymongo:PRIMARY> db.docs.find({_id:{$gte:10}})
{ "_id" : 10, "data" : 10 }
```
**如果使用 expr 比较的聚合查询，能够稳定地导出类型不同的数据：**
```
mymongo:PRIMARY> db.docs.aggregate([   { $match: { $expr: { $gte: ["$_id", 10] } } }, ])
{ "_id" : 10, "data" : 10 }
{ "_id" : "a", "data" : "a" }
{ "_id" : ISODate("2026-09-10T08:41:58.420Z"), "data" : "xxx" }
mymongo:PRIMARY> db.docs.aggregate([   { $match: { $expr: { $lte: ["$_id", 10] } } }, ])
{ "_id" : 1, "data" : 1 }
{ "_id" : 5, "data" : 5 }
{ "_id" : 10, "data" : 10 }
```

原理在于，expr 走的比较函数不是 [ComparisonMatchExpression::matchesSingleElement](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/db/matcher/expression_leaf.cpp#L117)， 而是 [Value::compare](https://github.com/mongodb/mongo/blob/r8.0.9/src/mongo/db/exec/document_value/value.cpp#L676
), 这个函数能够支持不同类型的比较操作。

上述方法，在 4.0, 4.2, 4.4, 5.0, 8.0 等多个版本均测试成功。     
而且在官方的 [mongot](https://github.com/mongodb/mongot/blob/v1.70.4/src/main/java/com/xgen/mongot/util/mongodb/CollectionScanAggregateCommand.java#L216
) （基于 lucene 的全文检索服务）中，也使用这种方式进行断点续传。

但是需要注意的是，从 [MongoDB 5.0](http://mongodb.asprain.cn/v5.3/release-notes/5.0/#-expr-operator--comparison-operators-use-indexes)开始，放置在 `$expr` 运算符中的 `$eq`、`$lt`、`$lte`、`$gt` 和 `$gte` 运算符可以使用索引来提高性能。
对于**低于 5.0 的内核版本，请谨慎使用。** 

基于此，可以使用 splitVector 分块之后，使用 expr 比较的方式，实现并行导出。    

## 5.4 使用 Clustered Collection
...
2. MongoDB 的比较操作符只支持返回同类型的数据，因此传统的分片后走 indexScan 导出的方式可能存在“类型陷阱”。可以使用 expr 比较的聚合操作规避“类型陷阱”，但请注意只有从 5.0 内核版本开始才支持走索引的执行计划，低版本内核会走全表扫描导致性能低下，需要仔细甄别。       
...
9. https://github.com/mongodb/mongot/blob/v1.70.4/src
10. http://mongodb.asprain.cn/v5.3/release-notes/5.0/#-expr-operator--comparison-operators-use-indexes
