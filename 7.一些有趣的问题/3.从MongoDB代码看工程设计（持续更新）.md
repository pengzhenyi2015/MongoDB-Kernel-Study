...
每次内存 page 会根据上述 precomp 值进行分隔。然后在压缩并写入硬盘后，根据实际压缩大小以及数据原始大小，再去更新 precomp 值。    
...
## 2.6 生命周期管理：嵌入式引用计数    
在 C++ 中， shared_ptr 通过引用计数来实现生命周期管理，当引用计数为 0 时，自动释放对象。       
但是 shared_ptr 需要独立维护一个控制块管理引用计数，在一定程度上可能造成性能损失。    
不过需要说明的是, `std::make_shared` 在效率上有所提升，参考 [C++手册](https://en.cppreference.com/w/cpp/memory/shared_ptr/make_shared):    

<p align="center">
<img width="2216" height="158" alt="image" src="https://github.com/user-attachments/assets/3d51fdd5-553c-4f94-8f7c-b557d5b9e3f9" />
</p>

而 `boost::intrusive_ptr` 采用了嵌入式的引用计数方式，控制块直接被包含在对象中。由于控制块的分配和释放和对象一起完成，而且空间上连续，因此理论上性能会高一些。    

MongoDB 中，很多数据结构都采用了 `boost::intrusive_ptr` 来管理生命周期，如聚合操作的核心数据结构 DocumentSource 等。为了实现这一点，内核中定义了 [RefCountable](https://github.com/mongodb/mongo/blob/r4.2.25/src/mongo/util/intrusive_counter.h#L42) 基类，定义了一个原子变量用于引用计数，并暴漏了 intrusive_ptr_add_ref 和 intrusive_ptr_release 接口用于引用计数的操作。所有希望使用 `boost::intrusive_ptr` 进行生命周期管理的类，只需要继承 RefCountable 即可。    
```
class RefCountable {
...
- 资源获取在构造函数里做    
- 资源释放在析构函数里做    
- 离开作用域时，对象自动析构，资源自动释放    

最常见的例子：std::lock_guard<mutex> - 构造时锁，析构时解锁。

### ScopeGuard 把 RAII 通用化
std::lock_guard 只能做解锁。如果你想做**任意的退出清理**呢？
ScopeGuard 的思想就是：     
**把任意一段代码（回调）包装成 RAII 对象，离开作用域时自动执行这段代码。**     
这样不管你怎么离开（正常返回/提前返回/抛异常），这段代码一定执行。    
从设计目标来看，ScopeGuard 类似 Golang 中的 **defer**，以及 Java 中的 **finally**.

### 解决什么问题
|问题	|ScopeGuard 怎么解决|
|--|--|
|多个出口都要写相同清理代码	|只写一次，自动在所有出口执行|
|加新出口忘记写清理代码	|不会忘记，自动执行|
|异常退出时资源泄漏	|异常导致栈展开，析构还是会执行 → 不会泄漏|

### ScopeGuard 和 ON_BLOCK_EXIT 的设计和使用
ScopeGuard 是一个 RAII 类，而 ON_BLOCK_EXIT 是基于此封装的宏定义，方便更便捷地使用。
实现代码位于 scopeguard.h 中，如下：
```
template <typename F> // 模板参数 F 是可调用对象的类型（通常是 lambda）
class [[nodiscard]] ScopeGuard { // 告诉编译器：如果你不用这个变量，给个警告。因为我们创建它就是为了析构，不能不小心扔了
public:
...
// 编译器自动推导出 F = std::decay_t<decltype(lambda)>
template <typename F>
...
// MONGO_SCOPEGUARD_ANON 用行号生成唯一变量名
// ON_BLOCK_EXIT([&] { ... }); 展开后就是：
// ScopeGuard onBlockExit1159 ([&] { ... });
// 为什么拼行号：同一个函数里可以写多个 ON_BLOCK_EXIT，每个变量名要唯一，不能重定义
#define ON_BLOCK_EXIT(...) auto MONGO_SCOPEGUARD_ANON(onBlockExit) = makeGuard(__VA_ARGS__)
```

**ScopeGuard 的使用示例（适合需要 dismiss 的场景）**    
```
void example() {
    // 1. 进入作用域，创建 ScopeGuard
    ScopeGuard guard([&]{ 
        printf("exit\n");  // ← 这是你要在出口执行的代码
    });
    ...
    if (error) {
        // 3a. 提前返回 → guard 析构 → 执行回调 → printf("exit\n")
        return;
    }
    ...
    if (success) {
        // 不需要执行了 → 撤销
        guard.dismiss();
        // 3b. 正常返回 → guard 析构 → 因为 dismiss 了 → 不执行
        return;
    }
    
    // 4. 正常结束 → guard 析构 → 执行回调 → printf("exit\n")
}
```

**ON_BLOCK_EXIT 的使用示例(适合不需要 dismiss 的场景)**    
```
void AsioTransportLayer::_runListener() noexcept {
    ...
    if (_isShutdown || _listener.state == Listener::State::kShuttingDown) {
        return;  // ← 提前返回！但是 ON_BLOCK_EXIT 的回调还是会执行！
    }
