...
5. 安全。包括认证、授权、审计、加密等安全机制的实现。
6. 最佳实践。包括一些业务的 MongoDB 使用经验，以及使用过程中经常会碰到的坑。
7. 一些有趣的问题。主要来源于平时工作中和同事茶余饭后探讨的一些有趣话题。
...
2. MongoDB 内核版本的迭代速度很快，很多模块都会不断进行优化和重构，并增加新功能。因此，本书中涉及到的原理和代码分析会注明版本，并给出对应的代码链接（以 4.x 和 5.0为主, 对于6.0-8.0引入的新特性也会特殊标注，也会适当给出 PostgreSQL 和 MySQL 的横向对比）。如果引用了第 3 方的资料，也会通过链接注明出处。
...
5. Gitee 同步镜像（适合不方便访问 github 的读者，而且国内的 github 可能会由于很多文档太大而无法展示）：[https://gitee.com/pengzhenyi2015/MongoDB-Kernel-Study](https://gitee.com/pengzhenyi2015/MongoDB-Kernel-Study)
