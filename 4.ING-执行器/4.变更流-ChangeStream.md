...
但是由于其单线程模型的设计，天生存在一定的性能瓶颈。特别是一些写并发高的场合（比如 DTS 中的大规模分片集群的同步），直接采用原生的 ChangeStream 方案可能表现不佳。此时可以考虑在架构上进行一些改造，将 ChangeStream 中的一些功能（比如拉去、排序合并等）进行并行化处理来满足业务需求。
