...
- 在 MySQL 中，可以通过 SELECT ... FOR UPDATE 锁定读来解决。   
- 在 PostgreSQL 中，可以通过 SERIALIZABLE 隔离级别来解决，依赖数据库的可串行化调度检测来发现上述 2 个事务的读写依赖。    
- 在 MongoDB 中，需要通过其他方式制造锁冲突或者写冲突。比如引入一个分布式锁。
