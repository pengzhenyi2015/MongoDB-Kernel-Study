...
对于 WorkingSet::_yieldSensitiveIds 和 WorkingSetMember::_isSuspicious，这里需要额外说明一下为啥这样设计。    
...
因此，需要将 indexScan（idHack, distinct 等其他使用索引的 stage 不再赘述） 得到的中间结果[记录在 WorkingSet::_yieldSensitiveIds](https://github.com/mongodb/mongo/blob/r4.2.5/src/mongo/db/exec/working_set.cpp#L96) 中，在执行器被 yield 出去之前，会遍历 WorkingSet::_yieldSensitiveIds 并将其中的 WorkingSetMember::_isSuspicious [设置为 true](https://github.com/mongodb/mongo/blob/r4.2.5/src/mongo/db/exec/working_set_common.cpp#L43).
