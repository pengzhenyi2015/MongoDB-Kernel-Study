...
如果带入 MongoDB 开发人员的角色，我们可以尝试对文档模型的选择过程进行如下推演：

<p align="center">
	<img width="1236" height="424" alt="image" src="https://github.com/user-attachments/assets/6cc2fa16-f0f2-49cf-bda3-651ea225eb8b" />
</p>
...
<p align="center">
  <img src="https://github.com/pengzhenyi2015/MongoDB-Kernel-Study/assets/16788801/c8f5f785-bf59-4655-b8bb-0abd0a8740e9" width="550">
</p>
...
<p align="center">
  <img src="https://github.com/pengzhenyi2015/MongoDB-Kernel-Study/assets/16788801/5b7ca0de-c3ca-4242-9928-996a4f285441" width="550">
</p>
...
# 5. 一切皆 BSON
BSON 在 MongoDB 中的使用是全流程的，从客户端代码将数据序列化成 BSON 开始，到网络传输，再到服务端的解析、执行计划，存储引擎的存储。    
...
>JSONB 是 PostgreSQL 中的二进制 json 类型。不同于 BSON 的是，JSONB 在头部维护了 key 和 value 的元数据索引，并且 key 的元数据数组会按照 key 的长度进行排序，这样比较方便进行二分查找。即通过 key 快速找到 key 和 value 的元数据，然后快速找到 key 和 value 的值。     
理论上来说，由于头部创建了“索引”，查找某个 key-value 的能力，JSONB 要比 BSON 更优。
但是缺点是一般存储空间更大，而且序列化和反序列化的开销要大于 BSON.

## MongoDB 使用 BSON
使用客户端连接 MongoDB 后执行插入命令`db.coll1.insert({"name": "zhangsan", "age": 35})`。 对应的网络包如下，可以发现客户端到服务端传输的是 BSON 二进制数据：

<p align="center">
<img width="756" height="661" alt="image" src="https://github.com/user-attachments/assets/b36f47ec-57a1-4e5e-944e-ae8dd4d186fe" />
</p>

>说明：基于 4.2.25 版本测试

## PostgreSQL 使用 JSONB
使用客户端连接 PostgreSQL 后执行插入命令` insert into test_table1(data) values ('{"name": "zhangsan", "age":35}'::jsonb);`。对应的网络包如下，可以发现客户端到服务端传输的是 JSONB UTF8 字符串数据：

<p align="center">
<img width="789" height="479" alt="image" src="https://github.com/user-attachments/assets/2cbee2a5-5cff-4498-b404-aefb36393d51" />
</p>
