...
>**值得注意的知识点**：     
WiredTiger 的 "写时复制" 机制，和 PostgreSQL 脏页的 "原地更新" 机制的不同，除了导致 Checkpoint 行为不一样之外，对 WAL/journal 日志的使用也不相同。
对于 PostgreSQL 来说，**需要保证刷下去的脏页，其对应的 WAL 日志已经先一步持久化到了磁盘**。这也正式 Write-Ahead-Log 的本意。
举例来说：
t1 时刻，对 Page_A 进行了修改，生成了对应的 WAL 记录 WAL_A，并将 Page_A [头部的 LSN](https://github.com/postgres/postgres/blob/REL_17_STABLE/src/include/storage/bufpage.h#L158 ) 设置为WAL_A 的结束位置 WAL_A_LSN;
t2 时刻，对 Page_B 进行了修改，生成了对应的 WAL 记录 WAL_B，并将 Page_B 头部的 LSN 设置为 WAL_B_LSN (且 WAL_B_LSN > WAL_A_LSN);
t3 时刻，对 Page_C 进行了修改，生成了对应的 WAL 记录 WAL_C，并将 Page_C 头部的 LSN 设置为 WAL_C_LSN (且 WAL_C_LSN > WAL_B_LSN);
t4 时刻，有个后台进程，将 WAL 日志持久化到了 FLUSH_LSN >= WAL_A_LSN, 且 FLUSH_LSN < WAL_B_LSN < WAL_C_LSN；
t5 时刻，后台刷脏页进程，找到 Page_A， 发现其头部的 LSN <= FLUSH_LSN，则可以写入到磁盘中；然后又找到了 Page_B 和 Page_C， 发现它们头部的 LSN > FLUSH_LSN，则[先刷日志，然后再刷脏页](https://github.com/postgres/postgres/blob/REL_17_STABLE/src/backend/storage/buffer/bufmgr.c#L3913)；

>如果在 t5 时刻，刷脏页进程不进行 LSN 判断，那么可能会由于先找到 Page_C 并将其写入磁盘，然后还没有来得及找到 Page_B 进程就退出了。
那么异常重启之后，数据库包含更新后的 Page_A 和 Page_C 页面，但是不包含 Page_B，由于 WAL 日志也只持久化了对 Page_A 的修改，此时就出现了数据不一致。

>对于 WiredTiger 来说，每次重启恢复，都是物理上快速恢复到上一个 checkpoint 点（类比上述例子，Page_A,B,C 的修改全部还原），然后再进行 journal 回放（修改 Page_A）。
因此，WiredTiger 刷脏页流程没有 LSN 判断，甚至 Page 上都不会存储最近修改的 LSN 信息。

>注：MySQL InnoDB 引擎的 redo log 和刷脏页机制，和 PostgreSQL 基本相同。
...
1. 不好处理压缩；
2. 不好处理大文档：    
2.1 以 PG Heap 引擎为例，默认超过 2KB 的数据会切片后存放到独立的 toast 表中。如果读取这些大数据，需要对 toast 表走 IndexScan 流程读所有切片数据，再拼接起来。这种方式下，性能会受到很大影响；    
2.2 以 MySQL InnoDB 为例，会有专门的溢出页存储这些大数据（仍属于当前表空间，不会创建额外的表和索引）。但是溢出页也需要满足 16KB 大小的限制，如果需要多个溢出页才能存储，会使用[链表](https://github.com/mysql/mysql-server/blob/mysql-8.0.41/storage/innobase/include/lob0lob.h#L50
)将多个溢出页串联起来。这种方式下，不用像 PG 走 IndexScan 流程，性能会更好一些；    
2.3 如果是 MongoDB WiredTiger 的变长页，也会使用溢出页存储这些大数据。但由于页是变长的，因此不需要多个溢出页串联。理论上性能最佳；       
...
8. https://github.com/mysql/mysql-server/blob/mysql-8.0.41
9. https://github.com/postgres/postgres/blob/REL_17_STABLE
